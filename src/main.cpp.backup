...
void setup()
{
  ...
  while (!Serial && millis() - t0 < 3000) { delay(10); }  // 최대 3초 대기
...
void performSafetyChecks()
{
  ...
  if (currentTime - lastSensorCheck > 5000) {
    ...
    bool ok = checkSensorHealth();
    if (!ok) {
      if (sensorFailStreak < 255) sensorFailStreak++;
    } else {
      sensorFailStreak = 0;
    }

    bool nowOk = (sensorFailStreak < 3); // 3회 연속 실패 시에만 오류 처리
    if (nowOk != lastSensorOk) {
      Serial.println(nowOk ? "센서 복구" : "WARNING: 센서 오류 감지");
      lastSensorOk = nowOk;
    }

    if (!nowOk && systemArmed) {
      systemArmed = false;
      stopAllMotors();
      Serial.println("센서 오류로 인한 자동 DISARM");
    }
